...
| **M6** | **SPA 工程草稿持久化（S7）** | 新增 `BffWorkspaceConfiguration`（`bff.workspace.dir`，默认 `./data/bff-workspace`）；`ProjectStore` 纯文件 IO（原子写 + 路径穿越双重防护 + 资产白名单 png/pgm/yaml/yml + 50MiB 上限）；OpenAPI 新增 `Projects` tag + 11 个 `/api/v1/projects/...` 路径（CRUD + draft GET/PUT + assets list/upload/download/delete）；`ProjectsHandler` + `ProjectAssetsHandler` + 4 个自定义异常映射；JsonMapper 注册 `JavaTimeModule` 支持 Instant；3 个测试类（ProjectStoreTest/ProjectsHandlerTest/ProjectAssetsHandlerTest，含 `docs/fixtures/opentcs-spa.draftV1.json` 金标准 round-trip）。 | 浏览器从 `/projects` 进入：新建/重命名/另存/删除工程；草稿自动 PUT 到 BFF；重启 BFF 仍能加载。 |
