...
- [x] **S5**：Point + Path 绘制 + 属性面板（首次引入 `pinia@^2.3.1`；`src/stores/project.ts` Pinia store 包 `background + points[] + paths[] + selection + pathDraftSrc`，debounce 200ms 落 `localStorage[opentcs-spa.draftV1]`；`useBackgroundMap` 退化为 thin compat shim；`src/domain/model/{types,naming,path}.ts` 镜像 `PointCreationTO` / `PathCreationTO`；`AnnotationLayer` 渲染 Point/Path + 拖拽 + 选中高亮；`HoverLayer` 加 Path 橡皮筋；`PropertyPanel` 编辑 name / type / 世界坐标m / orientation° / length mm / maxVelocity mm·s⁻¹ / maxReverseVelocity / locked；`EditorView` 接 `Delete` 删除 / `Esc` 取消 path 半态 / 空白处 click 清空选区）
- [x] **S6**：Location + Block + Vehicle（在 S5 的 `useProjectStore()` 上追加 `locationTypes[] / locations[] / blocks[] / vehicles[]`，`localStorage` envelope 升级到 v2 并向后兼容 v1；`src/domain/model/types.ts` 镜像 `LocationTypeCreationTO / LocationCreationTO / BlockCreationTO / VehicleCreationTO`，新增 `LocationRepresentation / BlockType` 枚举与 `EntityKind` 扩展；`src/domain/editor/tools.ts` 加 `location(O) / block(B) / vehicle(K)` 三种工具；`AnnotationLayer` 渲染 Location 方块 + Vehicle 带朝向矩形 + Location↔Point 虚线 link + 选中 Block 时画 member 包络框；`PropertyPanel` 按 kind 分发到 `src/components/property/{LocationTypeForm,LocationForm,BlockForm,VehicleForm}.vue` 四个子表单；Point/Location/Path 重命名 & 删除均级联更新到 Block.members / Location.links；面板右上角新增 `+ LocationType / + Block` 快捷新建）
- [x] **S7**：草稿持久化（BFF 文件存储）（BFF 新增 `ProjectStore` + 11 个 `/api/v1/projects/...` 路径：列表/新建/取/重命名/删除/另存为/取草稿/写草稿/资产列表+上传+下载+删除；落地到 `${bff.workspace.dir}/projects/{id}/{meta.json,draft.json,assets/}`，单个资产上限由 `bff.workspace.assetMaxBytes`（默认 50 MiB / 52428800 B）流式校验，原子写 `Files.move(ATOMIC_MOVE)`，路径穿越双重防护；SPA 新增 `useProjectsStore` + `ProjectsView` + `useCloudDraftSync` 500ms debounce PUT，`/` 重定向到 `/projects`，`/editor/:projectId?` 进入前 hydrate 草稿）
- [x] **S8**：发布到 Kernel（BFF `POST /api/v1/plant-models/publish` + `IntermediateJsonToPlantModelConverter` 镜像层 + `bff.kernel.{host,port,userName,password}` 配置；SPA `/projects/:projectId/publish` 路由 + `src/views/PublishView.vue`：左栏草稿计数与本地校验红条 / 右栏 dryRun + 确认发布 + 失败 toast 含 `code/message/fieldPath`）
- [x] **S9**：订单 + SSE 实时可视化（BFF `TransportOrder` 扩出 `state` + `processingVehicle`，`POST /api/v1/transport-orders` 已就位；SPA 新增 `src/api/endpoints/{transportOrders,sseEvents}.ts`、`src/stores/liveStatus.ts`（全局唯一 SSE 客户端，`vehicles`/`transportOrders`/`orderTimeline` 200 条环形缓冲）、`src/composables/useLiveVehicleOverlay.ts`（Kernel 实时态优先于草稿态，仅注入 `AnnotationLayer` Vehicle 数据源）、`src/views/OrdersView.vue`（多目的地表单 + NOP/MOVE/LIFT/DROP）、`src/components/OrderStatusSidebar.vue`；路由 `/projects/:projectId/orders`；`App.vue` 顶层启停 SSE；已知坑：BFF `Vehicle` 无 `precisePosition`/`orientationAngle`，画布只能在 Point 间"跳点"显示，订单历史未持久化刷新即丢）
- [ ] S10：收尾

每完成一个 S，请在本节做 `[ ]` → `[x]`，并在 `spa-frontend-architecture.md` 末尾追加一段「S{N} 完工备忘」（≤30 行：已完成模块、已知坑、下一阶段入口文件）——这是跨对话上下文的持久化锚点。

## 5. S9 验收用例草稿（2026-05-26）

| # | 场景 | 操作 | 期望 |
|---|---|---|---|
| 1 | Happy path | 进入 `/projects/:id/orders`，选车 → 配置 3 步目的地：先去某 Point（`MOVE`），再去某 Location 执行 `LIFT`，再到另一 Location 执行 `DROP`（或任意符合「Point⇒MOVE/PARK；Location⇒NOP/MOVE 或其 LocationType `allowedOperations` 中的操作」的组合）→ 提交 | toast 成功并显示新订单 `name`；自动回到 `/editor/:id`；2 秒内 Vehicle 矩形沿点序列移动，颜色随 `IDLE/EXECUTING/CHARGING/ERROR` 切换；右侧 `OrderStatusSidebar` 时间线出现 `RAW → ACTIVE → BEING_PROCESSED → FINISHED`。提示：OrdersView 已根据目标类型动态过滤操作下拉，避免出现 `BAD_REQUEST: <op> is not a valid operation for point destination ...` |
| 2 | SSE 断线重连 | 提交后停掉 BFF 5 秒再启动 | 画布不崩；`OrderStatusSidebar` 顶部 pill 显示「重连中…」；BFF 恢复后 pill 回到「已连接」，时间线继续滚动；指数退避在 `src/api/sse.ts` 已封装，无需新写 |
| 3 | 非法 destination 4xx | OrdersView 提交一个 `destination` 指向不存在的 Point | toast 显示后端 `code: message` 与 `fieldPath`（`{ toastOnError: false }` 后由 OrdersView 自己格式化）；表单不进入 loading 死锁，按钮恢复可点 |
| 4 | Kernel 502 toast 文案 | 临时让 BFF 与 Kernel 断开后提交订单 | toast 显示 `KERNEL_UNAVAILABLE: ... 502`（沿用 PublishView 同一套 `BffErrorResponse` 解析）；画布订阅继续工作（互不阻塞） |
