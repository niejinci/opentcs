...
| # | 场景 | 操作 | 期望 |
|---|---|---|---|
| 1 | Happy path | 进入 `/projects/:id/orders`，选车 → 配置 3 步目的地（含 1 次 LIFT + 1 次 DROP）→ 提交 | toast 成功并显示新订单 `name`；自动回到 `/editor/:id`；2 秒内 Vehicle 矩形沿点序列移动，颜色随 `IDLE/EXECUTING/CHARGING/ERROR` 切换；右侧 `OrderStatusSidebar` 时间线出现 `RAW → ACTIVE → BEING_PROCESSED → FINISHED` |
